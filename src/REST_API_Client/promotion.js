const PromotionAPI = (axiosInstance) => {
    ...
    const updatePromotion = async (orderId, data) => {
        const access_token = localStorage.getItem("access_token");
        console.log("data update", data);


        try {
            const res = await axiosInstance.patch(`/Promotions/update/${orderId}`, data, {
                headers: {
                    Authorization: `Bearer ${access_token}`,
    ...
    return {
        createPromotion,
        getAllPromotion, // Trả về phương thức getAllOrder
        updatePromotion
