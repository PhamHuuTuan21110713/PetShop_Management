...
const CategoryAPI = (axiosInstance) => {
  ...
  async function getCategoriesSale(filters) {
    try {
      const res = await axiosInstance.get(`/categories/categories-report`, {
        params: filters,  // filters là object chứa { month, year, quarter, ... }
      });
      return res.data;
    } catch (error) {
      if (error.response) {
