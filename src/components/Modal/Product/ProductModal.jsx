...
const DetailAccountModal = ({ open, onClose, product, onChange }) => {
  ...
  //       // setDesc(newProduct?.desc); setName(newProduct?.name); setType(newProduct?.type);
  //       // setPrice(newProduct?.price); setQuantity(newProduct?.quantity); setState(newProduct?.state);
  //       // defaultInfor.current = { ...newProduct };
  //       onChange(newProduct);
  //       onClose();
  //     })
  //     .catch(err => {
  //       console.log("loi cap nhat thong tin sản phẩm: ", err);
  ...
  //         imageFormData.append("image", image); // Giả sử "image" là file ảnh bạn muốn gửi

  //         // Gọi API để thêm ảnh thumbnail mới
  //         ProductFetch.addThumbnail(product._id, imageFormData)
  //           .then(() => {
  //             // Sau khi cập nhật ảnh, bạn có thể thực hiện các hành động cần thiết
  //             window.alert("Cập nhật ảnh thumbnail thành công!");
  //             // Cập nhật lại sản phẩm sau khi cập nhật ảnh
  //             onChange(newProduct);
  //             onClose();
  //           })
  //           .catch(err => {
  //             console.log("Lỗi cập nhật ảnh thumbnail:", err);
  //             window.alert(`Lỗi cập nhật ảnh thumbnail: \n ${err}`);
  //           });
  //       } else {
